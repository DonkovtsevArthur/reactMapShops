...
gulp.task("watch", () => {
  ...
  gulp.watch(settings.watch.font, gulp.series("fonts"));
  gulp.watch(settings.watch.browserSync).on("change", browserSync.reload);
});

gulp.task(
  "build.dev",
  gulp.series("clear", gulp.parallel("js", "css", "img", "fonts"))
);
gulp.task(
  "dev",
  gulp.parallel("watch", gulp.series("clear", "build.dev", "server"))
);
...
gulp.task(
  ...
  gulp.series(
    "clear",
    gulp.parallel("js.prod", "css.prod", "img.prod", "fonts")
  )
);
