...
export interface WidgetsActionCreators {
  getData(id: string): (dispatch: Redux.Dispatch<WidgetsActions>, getStore: () => Store) => Axios.AxiosPromise;
  getAllData(id: string): (dispatch: Redux.Dispatch<WidgetsActions>, getStore: () => Store) => Axios.AxiosPromise;
  getSource(): void;
...
// async function getMarkets(dispatch: Redux.Dispatch<Store>, widget: WidgetConfig, token: string) {
//   try {
//     dispatch({ type: action.REQUEST_MARKETS });
//     const query = getStoresQuery(widget);
//     const marketsResponse: Axios.AxiosResponse<Market[]> =
...
//     dispatch({ type: action.SUCCESS_MARKETS, markets });
//   } catch (error) {
...
async function getInfoAboutDashboard(dispatch: Redux.Dispatch<Store>, getStore: () => Store, dashboardId: string) {
  try {
    ...
    // let token = '';
    const widgetConfigs = await Promise.all(widgets.map(async (widget) => {
...
const widgetsActions: WidgetsActionCreators = {
    ...
    return async (dispatch: Redux.Dispatch<WidgetsActions>, getStore: () => Store) => {
      try {
        ...
        dashboardResponse.data.forEach((dashboard) => {
          if (dashboard.group === 'evotor-monitor-fran') {
            dispatch({
        ...
        const dashboardId = dashboardResponse.data.filter(dashboard => dashboard.active && dashboard.group === 'evotor-monitor-fran');
        if (dashboardId.length > 0) {
          if (dashboardId[0].copy) {
            const notCopyDashboard = dashboardResponse.data.filter(item => !item.copy && item.group === 'evotor-monitor-fran')[0];
        ...
        const myDashboard = dashboardResponse.data.filter(dashboard => !dashboard.copy && dashboard.group === 'evotor-monitor-fran')[0]._id;
  ...
  updateWidget(id, period) {
    return async (dispatch: Redux.Dispatch<WidgetsActions>, getStore: () => Store) => {
      ...
      const activeWidgetConfigs = widgets.map(item => widgetConfigs.filter(config => config.appId === item.i)[0]);
      ...
      await Promise.all(newWidgetConfigs.map(async (item) => {
  ...
  updateIndexDynamicWidgets(index, indexAction) {
    return async (dispatch: Redux.Dispatch<WidgetsActions>, getStore: () => Store) => {
      ...
      await Promise.all(updateWidgets.map(async (data) => {
        await reqData.post(API_PROXY_URL(appType), {
          path: 'widget/update',
          method: 'POST',
          data
      ...
      await Promise.all(updateWidgets.map(async (item) => {
        return widgetsActions.getData(item.appId)(dispatch, getStore);
      }));
    };
  },
