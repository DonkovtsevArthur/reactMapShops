  ...
  getTokens(appId) {
    return async (dispatch: Redux.Dispatch<Store>, getStore: () => Store) => {
      try {
        ...
        const response: AxiosResponse<Token[]> =
  ...
  share() {
    return async (dispatch: Redux.Dispatch<Store>, getStore: () => Store) => {
      try {
        ...
        const response: AxiosResponse<{accessToken: string}> = await reqData.post(API_PROXY_URL(appType), {
          path: 'access/share',
          method: 'POST',
          data: { orgId, apps }
        });
  ...
  remove(accessToken) {
    return async (dispatch: Redux.Dispatch<Store>, getStore: () => Store) => {
      try {
        ...
        const newTokens = olkdTokens.filter(item => item.accessToken === accessToken ? false : true);
        dispatch({ type: integration.SUCCESS_ON_TOKEN, tokens: newTokens });
      } catch (error) {
        dispatch({ type: integration.ERROR_ON_TOKEN, message: error.message });
        logger.error(error.message);
    ...
    return async (dispatch: Redux.Dispatch<Store>, getStore: () => Store) => {
      try {
        ...
        await reqData.post(API_PROXY_URL(appType), {
          path: 'access/copy',
          method: 'POST',
          data: { orgId, accessToken, alias, group: 'evotor-monitor-fran' }
        });
