...
class Container extends React.Component<DashboardProps, {}> {
  ...
  getWidgets = () => {
    ...
    if (userWidgets) {
      const userWidgetConfigs = widgetConfigs.filter(config => userWidgets.some(item => item.i === config.appId));
  ...
  getDashboards() {
    ...
    const dashboardsIds = Object.keys(this.props.source);
    const activeDashboard = dashboardsIds.filter(item => source[item].active)[0];
    if (source[activeDashboard].widgets) {
      ...
      if (status === 'success') {
        ...
        const period = this.props.widgetConfigs.filter(item => item.appId === activeWidget.i)[0].period;
        const hash = Hash.sha1({ period });
        if (!this.props.dashboards.data[hash] && this.props.dashboards.status === '') {
...
const mapDispatchToProps = (dispatch: Redux.Dispatch<Store>) =>
  Redux.bindActionCreators<any>({ ...widgetsActions, ...dashboardsActions, ...integrationActions }, dispatch);
