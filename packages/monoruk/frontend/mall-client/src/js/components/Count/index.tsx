...
const Count: React.StatelessComponent<LineProps> = props => {
  ...
  const graph = props.graphData.map((data, i) => {
    const prev =
      ((data.x as number) - data.x1) / ((data.x as number) + data.x1) * 100;
    if (data.y === '') {
      return null;
    }
    return (
        ...
        <div className="widget-counter__text">
          <div className="widget-counter__title">
            {data.y
              ...
              : getLabel(
                  '',
                  props.widgetConfig.index,
                  props.widgetConfig.indexAction
                ).title}
          </div>
          <div className="widget-counter__value">
            <p style={{
              ...
              {getLabel(
                ...
                props.widgetConfig.index,
                props.widgetConfig.indexAction
              ).measure === 'currency' ? <i className="fa fa-ruble" /> : null}
            </p>
            <span className={prev > 0 ? 'plus' : 'minus'}>
              {prev.toFixed(1) + '%'}
            </span>
          </div>
        </div>
