  ...
  getSource() {
    return async (dispatch: Redux.Dispatch<WidgetsActions>, getStore: () => Store) => {
      try {
        ...
        const dashboardId = dashboardResponse.data.filter(dashboard => dashboard.active && dashboard.group === GROUP);
        if (dashboardId.length > 0) {
          if (dashboardId[0].copy) {
            const notCopyDashboard = dashboardResponse.data.filter(item => !item.copy && item.group === GROUP)[0];
        ...
        const myDashboard = dashboardResponse.data.filter(dashboard => !dashboard.copy && dashboard.group === GROUP)[0]._id;
