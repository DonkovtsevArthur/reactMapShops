...
export type DashboardsParam = DashboardsStore & DashboardsActionCreators & DashboardsProps;
...
class Container extends React.Component<DashboardsParam, {}> {
  ...
  getData() {
    ...
    case '':
      const data = this.props.data[this.props.hash].filter(item => item.org === this.props.alias);
      let x: number = -1;
      let x1: number = -1;
      if (data.length === 1) {
        x = data[0].x;
          ...
          <Material.IconMenu
            iconButtonElement={
              <button
                ...
                disabled={this.props.alias === 'Мои магазины' || this.props.alias === 'Рабочий стол'}
              >
...
const Dashboards = connect<DashboardsStore, DashboardsActionCreators, DashboardsProps>(
  mapStateToProps,
  mapDispatchToProps
)(Container);
