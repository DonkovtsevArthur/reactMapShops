...
function formatDate(period: string) {
  switch (period) {
  case 'month':
    return Moment().add(-1, 'month').utc().format('YYYY-MM-DD HH:mm:00');
  case 'quarter':
    return Moment().add(-4, 'month').utc().format('YYYY-MM-DD HH:mm:00');
  case 'half-year':
    return Moment().add(-6, 'month').utc().format('YYYY-MM-DD HH:mm:00');
  case 'year':
    return Moment().add(-12, 'month').utc().format('YYYY-MM-DD HH:mm:00');
  default:
    return Moment().add(-1, 'month').utc().format('YYYY-MM-DD HH:mm:00');
  }
}
...
export function queryGeneratorDebtors5() {
  return `SELECT
          ...
          FROM ( SELECT * FROM dataSource ORDER BY timestamp )`;
}
...
export default (widget: WidgetConfig) => {
  ...
  return `SELECT
            ...
            toDate(x) as x
          FROM (
            SELECT dateDoc AS x, ${widget.index} AS y, timestamp FROM dataSource ${dateLimit} ORDER BY timestamp
          )
          GROUP BY x`;
};
