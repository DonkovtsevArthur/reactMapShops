...
import { VictoryLine, VictoryChart, VictoryAxis, VictoryTheme, VictoryScatter } from 'victory';
...
const Line: React.StatelessComponent<LineProps> = props => {
  ...
  const graph = (
    ...
    >
      <VictoryAxis fixLabelOverlap tickFormat={x => Moment(x).format('DD.MM.YYYY')} />
      <VictoryAxis
        fixLabelOverlap
        dependentAxis
      ...
      />
      <VictoryLine data={props.graphData} y="y" x="x" />
      <VictoryScatter
        ...
        labelComponent={<VictoryTooltip style={{ fontSize: 12, background: 'white' }} />}
      />
    </VictoryChart>
  );
  const empty = (
