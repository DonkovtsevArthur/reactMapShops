...
interface MarketsPropsChunk {
  stores: MarketsStore;
}

interface DashboardsPropsChunk {
...
class Container extends React.Component<DashboardProps, {}> {
  ...
  componentWillMount() {
    const [dashboardId, marketId] = this.props.routing.pathname
      .split('/')
      .slice(1);
  ...
  getWidgets = () => {
    ...
    if (activeWidgets) {
      const configs = widgetConfigs.filter(item =>
        activeWidgets.some(active => active.i === item.appId)
      );
  ...
  getMarkets(dashboardId: string) {
    if (this.props.source[dashboardId].widgets) {
      ...
      const config = this.props.widgetConfigs.filter(
        item => item.appId === widgetId
      )[0];
      if (config) {
        const period = config.period;
        return <Markets dashboardId={dashboardId} period={period} />;
      }
      return (
        <UI.ElementWindow>
          <Loader />
  ...
  getData() {
    ...
    if (
      source[dashboardId] &&
      source[dashboardId].widgets &&
      source[dashboardId].widgets.length > 0 &&
      this.props.status !== 'request' &&
      this.props.status
    ) {
      const dashboardName = source[dashboardId].copy
        ? source[dashboardId].alias
        : 'Свои данные';
      let nameMarket = '';
      if (market) {
      ...
      return (
        <>
          ...
          <div className="dashboard">
            ...
            {this.getWidgets()}
            {this.props.market || this.props.markets <= 1
              ? null
              : this.getMarkets(dashboardId)}
          </div>
        </>
      );
    }
