...
export type AccessProps = { widgets: WidgetsStore } & IntegrationStore &
  IntegrationActionsCreators;
...
class Container extends React.Component<AccessProps, {}> {
  ...
  componentWillMount() {
    ...
    if (
      dashboards[myDashboard].widgets &&
      dashboards[myDashboard].widgets.length > 0
    ) {
...
const Access = connect<IntegrationStore, IntegrationActionsCreators, {}>(
  mapStateToProps,
  mapDispatchToProps
)(Container);
