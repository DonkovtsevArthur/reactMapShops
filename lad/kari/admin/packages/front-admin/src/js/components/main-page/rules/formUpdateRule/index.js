...
class RuleUpdateForm extends Component {
	state = {
		...this.props.rule,
		actionOptions: [
	...
	render() {
		const {props: {closeId}, state: {actionOptions, targetOptions, rule}, handleSubmit, handleChange} = this
	...
	handleSubmit = () => {
		const {state, props: {updateRule, rule: {ruleId}}} = this
		updateRule({ruleId, rule: state})
	}
}
...
export default hot(module)(connect(mapStateToProps, mapDispatchToProps)(RuleUpdateForm))